 ...
 * requires: first is valid
 */
static bool check_win(const struct forest *frst) {
  assert(frst);
  // We need two counters for the ghosts
  int count_unrevealed = 0, count_ghost = 0;
  for (int i = 0; i < frst->width * frst->height; i++) {
    // Ghosts may remain partially or completely unmarked
    // Hence we check both possibilities
    if ((frst->player_map[i] == UNREVEALED) || (frst->player_map[i] == MARKED))
      count_unrevealed++;
    // We also count the number of ghosts we inputted
    if (frst->ghost_map[i] == ghost_presence)
      count_ghost++;
  }
  // Check for equality
  return (count_unrevealed == count_ghost);
}
...
int forest_reveal(struct forest *frst, int x, int y) {
  assert(frst);
  // If our position is valid return the invalid symbol
  if (!check_valid_pos(frst, x, y)) return INVALID_SYMBOL;

  int flattened_position = transfer_2D_into_1D_pos(frst, x, y);
  // If we encounter the ghost, game over, no need to mutate, and we reveal the ghost
  if (frst->ghost_map[flattened_position] == ghost_presence) {
    frst->player_map[flattened_position] = ghost_presence;
    return forest_reveal_dead;
  }
  else {
    // We did not die
    // Make the recursive call to uncover the map
    mutate_player_map_pos(frst, x, y);
    // Did we win?
    if (check_win(frst))
      return forest_reveal_win;
    // If not, keep going
    else
...
int main(void) {
  ...
  /*  The rest of main is to generate the .exe version for students  */
  return 0;
}
