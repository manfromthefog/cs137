...
/***
 * lookBehind()
 * Removes a substring from a word preceded by another substring.
 * 
...
void lookBehind(const char* check, const char* s_sub, char* word) {
    ...
    for (int i = 0; word[i] != '\0'; i++) {
        ...
        if (substring_at_index(check, word, i)) {
            ...
            // We check that "sub-word" for the substring "recursively"
            // Not actual recursion, but we keep checking if we can remove the substring
            while (substring_at_index(s_sub, word, lower_bound)) {
